...
Below is the pseudocode for how our agent model is used in the <dt-cite key="openai_gym">OpenAI Gym</dt-cite> environment. Running this function on a given `controller` C will return the cumulative reward during a rollout of the environment.
...
2. Train VAE (V) to encode frames into latent vector $z \in \mathcal{R}^{32}$.
